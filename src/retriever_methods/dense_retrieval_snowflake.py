...
# -*- coding: utf-8 -*-
"""
Dense Retrieval with BGE-M3 on a CSV VectorDB and JSONL Questions.

- Loads document embeddings from a CSV (columns: doc_id, text, embedding as JSON list).
- Loads query encoder model name from a config JSON (e.g., {"model_name": "BAAI/bge-m3", ...}).
...
class QueryEncoder:
    """
    Wraps BGEM3 encoders. Prefers FlagEmbedding, falls back to SentenceTransformers.
    """
    ...
    def __init__(
        ...
        self.backend = None  # "flag" or "sbert"
        self.model = None
        ...
        try:
            ...
            if device and device != "auto":
                # BGEM3FlagModel uses "use_fp16" and automatically picks device; no direct device string.
                pass
            ...
            try:
                ...
                raise RuntimeError(
                    ...
                    f"Install either 'FlagEmbedding' or 'sentence-transformers'. Original error: {e}"
                )
    ...
    def encode_queries(
        ...
        if instruction is None:
            # Default instruction for BGE family queries.
        ...
        if self.backend == "flag":
            # BGEM3FlagModel encode for queries. Return only dense vectors.
            out = self.model.encode_queries(
                q_texts,
                return_dense=True,
                return_sparse=False,
            ...
            q_vecs = np.array(out["dense_vecs"], dtype=np.float32)
        else:
            # SentenceTransformer
...
def main():
    p = argparse.ArgumentParser(description="Dense Retrieval with BGE-M3")
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="JSON with at least {'model_name': 'BAAI/bge-m3'}",
    )
    p.add_argument(
    ...
    model_name = cfg.get("model_name") or cfg.get("model") or "BAAI/bge-m3"
