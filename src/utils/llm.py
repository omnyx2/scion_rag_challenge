...
class LLMClient:
    ...
    def send_request(self, prompt, model, temperature, max_tokens, top_p, num_samples, max_retries=3):
    ...
    # 내가 만듬
    def generate_sub_laber(self, idx, prompt, num_samples, model=None, temperature=0.7, max_tokens=800, top_p=1, ignore_cache_samples=False):
        print(idx, ':requested')
        result = self.generate(prompt, num_samples, model=model, temperature=temperature, max_tokens=max_tokens, top_p=top_p, ignore_cache_samples=ignore_cache_samples)
        return {"id": idx, "response": result}
    ...
    def generate_parallel(self, prompts, num_samples, model=None, temperature=0.7, max_tokens=800, top_p=1, num_workers=8):
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            ...
            return results
    # 내가 만듬
    def generate_parallel_prompts_manager(self, promp_histories, num_samples=1, model=None, temperature=0.7, max_tokens=800, top_p=1, num_workers=8):
        """use concurrent futures to generate samples in parallel"""
        num_samples = 1
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            futures = [executor.submit(self.generate_sub_laber, prompt_id, prompt, num_samples, model, temperature, max_tokens, top_p) for prompt_id, prompt in promp_histories.items()]
            results = []
            for future in tqdm(as_completed(futures), total=len(futures), desc="Generating samples"):
                results.append(future.result())
            return results
